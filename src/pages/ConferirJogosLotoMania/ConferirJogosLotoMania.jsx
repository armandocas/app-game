...
import "./ConferirJogosLotoMania.css";

const ConferirJogosLotoMania = () => {
  ...
  const toggleNumberSelection = (number) => {
    ...
    } else if (selectedNumbers.length < 50) {
      const newSelection = [...selectedNumbers, num];
      setSelectedNumbers(newSelection);
      const matches = newSelection.filter((n) => sortedNumbers.includes(n));
      setMatchCount(matches.length);
    } else {
      toast.error("Você só pode selecionar 50 números!", {
        position: "top-center",
        autoClose: 2000,
      });
    }
  ...
  return (
    <div className="conferir-jogos-container">
      <ToastContainer />
      <h1>Conferir Jogos da LotoMania</h1>
...
export default ConferirJogosLotoMania;
