...
import "./GenerateFromDataLotoMania.css";

const GenerateFromDataLotoMania = () => {
  ...
  const generateNumbersFromData = () => {
    ...
    const remainingNumbers = Array.from({ length: 100 }, (_, i) => i + 1).filter(
      (num) => !dataNumbers.includes(num)
    );

    while (dataNumbers.length < 50) {
  ...
  return (
    <div className="generate-numbers-container">
      <ToastContainer />
      <h1>Gerar Números com Base em Dados - LotoMania</h1>
...
export default GenerateFromDataLotoMania;
