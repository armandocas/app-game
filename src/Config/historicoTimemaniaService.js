...
async function coletarDadosTimemania() {
  ...
  try {
    ...
    for (let id = ultimoId + 1; id <= ultimoId + 2200; id++) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          const documento = {
            ...
            time_coracao: {
              nome: dados.tcn || "",
              valor: dados.tcv || "0,00",
              ganhadores: dados.tcw || "0",
            },
          ...
          await setDoc(
            doc(collection(db, "historico_timemania"), String(dados.s)),
            documento
          );
...
export default coletarDadosTimemania;
